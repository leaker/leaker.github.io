...
def revise_header_content(header_lines: list[str]):
    ...
    for i, line in enumerate(header_lines):
        # date line
        rdate = re.search(
            r"date:\s+(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})", line, re.M
        )
        if rdate:  # date line for need revise to TZ format
            header_lines[i] = (
                f"date: {rdate.group(1)}-{rdate.group(2)}-{rdate.group(3)}T{rdate.group(4)}:{rdate.group(5)}:{rdate.group(6)}{GMT}\n"
            )
            revised = True
    ...
    if header_revised:
        header_lines = yaml.dump(
            header, default_flow_style=False, allow_unicode=True
        ).splitlines(True)
        revised = True
